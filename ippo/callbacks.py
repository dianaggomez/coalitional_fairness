...
class MultiAgentHLCCallbacks(DefaultCallbacks):
    def __init__(self):
        ...
        self._is_time = defaultdict(list)
        self.r_f = defaultdict(list)
        self._is_success_buffer = defaultdict(list)
        print("initialized")
    ...
    def on_episode_step(
        ...
        for agent_id in active_keys:
            ...
            if info:
                if "fair_reward" not in info:
                    continue
                #TODO: need to add time into the info for each agent
                episode.user_data["fair_reward_{}".format(k)].append(fair_reward)
    ...
    def on_episode_end(
        ...
        **kwargs
        ...
        for k in keys:
            info = episode.last_info_for(k)
            maybe_is_success = info.get("cleared_queue")
            ...
            if maybe_is_success is not None:
                try:
                    self._is_success_buffer[k].append(maybe_is_success)
                    if maybe_is_success:
                        # self._is_success_buffer.append(maybe_is_success)
                        self._is_time[k].append(maybe_time)
                        # log the reward associated with fairness
                    # self._is_queue.append(maybe_queue)
                except:
                    print("Dones with errors: ", maybe_time)
                    
            # TODO: Need to make sure that this is for 2 agents
            episode.custom_metrics["success_rate"][k] = np.mean(self._is_success_buffer[k])
...
class MultiAgentDrivingCallbacks(DefaultCallbacks):
    ...
    def on_episode_end(
        ...
        **kwargs
        ...
        # this is getting the mean over all agents
